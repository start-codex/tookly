<script lang="ts">
	...
	// local reactive list so the board updates without reload
	let localStatuses = $state<Status[]>([...data.statuses]);
			...
			<form onsubmit={handleCreate} class="flex flex-col gap-4 p-6">
				...
				<div class="flex flex-col gap-1.5">
					...
					>
						...
						<option value="in_progress">{t.catInProgress}</option>
